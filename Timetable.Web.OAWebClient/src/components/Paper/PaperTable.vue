<template>
  <div>
    <ElTable :data="tableData" highlight-current-row row-key="Id">
      ...
      <ElTableColumn prop="Url" label="跳转地址" show-overflow-tooltip>
  ...
  methods: {
    ...
    del(row) {
      if (this.hasDeletePermission) {
        this.$confirm("确认删除该问卷地址？", "提示", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning",
        })
          .then(() => {
            var url = this.$baseURL + "/Paper/Delete?id=" + row.Id;
    ...
    reload(e) {
      ...
      this.$nextTick(() => {
        ...
        var url = this.$baseURL + "/paper/list";
        if (url) {
          var data = {
