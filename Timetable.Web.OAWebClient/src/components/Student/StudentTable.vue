...
export default {
  ...
  computed: {
    ...
    hasResetPermission() {
      return this.hasPermission("SU_R");
    },
    hasDeletePermission() {
      return this.hasPermission("SU_D");
    },
  },
  mounted() {
    this.reload();
  ...
  methods: {
    ...
    deleteitem(row) {
      ...
      this.$confirm("此操作将永久删除学生账号, 是否继续?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }).then(() => {
    ...
    edit(e) {
      ...
      d.ClassText=[e.CollegeName,e.DepartmentName,e.AdministrativeClassName].filter(s=>s).join('-');
      delete(d.AdministrativeClassName);
      delete(d.SchoolOpenId);
      delete(d.AvatarUrl);
      delete(d.CollegeName);
      delete(d.DepartmentName);
      delete(d.CreateTime);
      this.$emit("openEdit", {
        ...d,
